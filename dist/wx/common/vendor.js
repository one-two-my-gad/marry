...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__config__ = __webpack_require__(85);
...
var db = wx.cloud.database({
  env: __WEBPACK_IMPORTED_MODULE_1__config__["a" /* CLOUD_CONFIG */].DBID
});

var add = function add(data, databseName) {
  return db.collection('' + databseName).add({
    data: data
  });
};

var get = function get(databseName, page) {
  return db.collection(databseName).skip(page * 10).limit(10).get();
};

var getBy = function getBy(data, databseName) {
  return db.collection('' + databseName).where(data).get();
};

var getOne = function getOne(id, databseName) {
  return db.collection('' + databseName).doc(id).get();
};

var update = function update(data, id, databseName) {
  return db.collection('' + databseName).doc(id).update({
    data: data
  }).catch(console.error);
};

var remove = function remove(id, databseName) {
  return db.collection('' + databseName).doc(id).remove();
};

var uploadFile = function uploadFile(cloudPath, filePath) {
...
var deleteFile = function deleteFile(fileList) {
  wx.showLoading({
    title: '\u6587\u4EF6\u5220\u9664\u4E2D\u2026',
    mask: true
  });
...
var getTempFileURL = function getTempFileURL(fileList) {
  wx.showLoading({
    title: '\u751F\u6210\u8FDE\u63A5\u4E2D\u2026',
    mask: true
  });
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return CLOUD_CONFIG; });
var CLOUD_CONFIG = {
  DBID: 'test1-4gqpf0yy03d701f7',
  DBNAME: 'baemon'
};
